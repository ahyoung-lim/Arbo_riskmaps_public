base_map <- function(world, lake_mask=T){
 # world map
  ...
  ggplot()+
  geom_sf(data=world, fill="white", color="grey70")+
  theme_bw()+
...
plotRaster <- function(raster) { 
  ...
  ggplot()+
    ...
    theme(panel.border = element_blank(),
        ...
        axis.text.y=element_blank(),
        axis.ticks=element_blank(),
        legend.title = element_text(family = "Arial", vjust = 2.5),
...
plotFRaster <- function(raster) { 
  ...
  ggplot()+ 
   ...
   scale_fill_manual(name = "",
                     values = cols, 
                     breaks = c("4", "3", "2", "1"),
                     labels = c("Both high", "New high", "Previous high", "Both low"), na.value = "white")
