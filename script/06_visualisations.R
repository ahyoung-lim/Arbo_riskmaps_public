...
arbo_occ_thinned$disease <- factor(
  arbo_occ_thinned$disease,
  levels = c("dengue", "zika", "chikungunya", "yf")
)
arbo_occ_thinned$Type = c("Polygon", "Point")[
  ((arbo_occ_thinned$Admin == -999) + 1)
]
...
(arbo_bar <- arbo_occ_thinned %>%
  ...
  ggplot() +
  geom_bar(
    aes(x = as.numeric(Year), y = n, fill = disease),
    color = "grey20",
    stat = "identity",
    position = position_stack(reverse = TRUE)
  ) +
  scale_fill_manual(
    values = c("#EE6677", "#228833", "#4477AA", "#AA3377"),
    name = "Disease",
  ...
  xlab(NULL) +
  ylab("Number of new unique occurrence points") +
  labs(tag = "a") +
  theme(
    ...
    legend.text = element_text(size = 16, family = "sans", vjust = 0.5),
    legend.text.align = 0,
...
ggsave(
  filename = paste0("outputs/Figures/Fig1a.pdf"),
  arbo_bar,
  height = 6,
  width = 19.2,
  dpi = 300
...
(dengue_data_plot <- base_map(world) +
  geom_point(
    data = subset(arbo_occ_thinned, disease == "dengue"),
  ...
  theme(
    ...
    legend.text = element_text(size = 10, family = "sans", vjust = 0.5),
    legend.text.align = 0,
    legend.position = "top",
...
(zika_data_plot <- base_map(world) +
  geom_point(
    data = subset(arbo_occ_thinned, disease == "zika"),
    color = "#4477AA",
    aes(x = Longitude, y = Latitude, shape = Type),
    alpha = 0.5,
    size = 2
...
(chik_data_plot <- base_map(world) +
  geom_point(
    data = subset(arbo_occ_thinned, disease == "chikungunya"),
    color = "#228833",
    aes(x = Longitude, y = Latitude, shape = Type),
    alpha = 0.5,
    size = 2
...
(yf_data_plot <- base_map(world) +
  geom_point(
    data = subset(arbo_occ_thinned, disease == "yf"),
    color = "#AA3377",
    aes(x = Longitude, y = Latitude, shape = Type),
    alpha = 0.5,
    size = 2
...
ggsave(
  filename = paste0("outputs/Figures/Fig1b_e.pdf"),
  fig1b_e,
  height = 9.6,
  width = 19.2,
  dpi = 300
...
fig2 <- plotRaster(Ras_Surv) +
  ...
  theme(legend.title = element_text(family = "sans", vjust = 2.5))
fig2

ggsave("outputs/Figures/Fig2.pdf", fig2, height = 6, width = 12, dpi = 300)
...
fig3 <- r_DCZ +
  ggtitle("a  Dengue, chikungunya, and Zika") +
  r_YF +
  ggtitle("b  Yellow fever") +
...
ggsave(
  filename = "outputs/Figures/Fig3.pdf",
  fig3,
  bg = "white",
  height = 10,
  width = 12,
...
ggsave(
  filename = "outputs/Figures/Fig4.pdf",
  fig4,
  bg = "white",
  height = 9,
  width = 18,
