...
world$continent <- ifelse(
  world$continent %in% c("North America", "South America"),
  "Americas",
  world$continent
)
...
continent <- world %>%
  ...
  group_by(continent) %>%
  summarise(
    dcz = round(sum(dcz_risk_pop, na.rm = T) * 1e-9, 2),
...
write.csv(
  table,
  paste0("outputs/Tables/Table1_Pop_at_risk_continent.csv"),
  row.names = F
)
...
country_at_risk <- world %>%
  st_drop_geometry() %>%
  group_by(standard_sovereignt, continent) %>%
...
table2 <- country_at_risk %>%
  mutate(
    ...
    tot_pop = as.numeric(format(
      round(tot_pop * 1e-6, 3),
      nsmall = 3,
      trim = FALSE
    )),
    pop_dcz = as.numeric(format(
      round(sum_dcz_risk_pop * 1e-6, 3),
      nsmall = 3,
      trim = FALSE
    )),
    pop_dcz_lwr = as.numeric(format(
      round(sum_dcz_lwr_pop * 1e-6, 3),
      nsmall = 3,
      trim = FALSE
    )),
    pop_dcz_upr = as.numeric(format(
      round(sum_dcz_upr_pop * 1e-6, 3),
      nsmall = 3,
      trim = FALSE
    )),

    pop_yf = as.numeric(format(
      round(sum_yf_risk_pop * 1e-6, 3),
      nsmall = 3,
      trim = FALSE
    )),
    pop_yf_lwr = as.numeric(format(
      round(sum_yf_lwr_pop * 1e-6, 3),
      nsmall = 3,
      trim = FALSE
    )),
    pop_yf_upr = as.numeric(format(
      round(sum_yf_upr_pop * 1e-6, 3),
      nsmall = 3,
      trim = FALSE
    )),
...
# remove latin accent
table2$Sovereignt <- stri_trans_general(
  str = table2$Sovereignt,
  id = "Latin-ASCII"
)

write.csv(
  table2,
  paste0("outputs/Tables/STable_Countries_at_risk.csv"),
  row.names = F,
  fileEncoding = "UTF-8"
)
