...
SI_fig1 <- ggarrange(p1, p2, p3, p4, nrow = 2, ncol = 2, common.legend = T)

ggsave(
  filename = paste0("outputs/Figures/SI_Fig1.png"),
  SI_fig1,
  bg = "white",
  height = 9,
  width = 18,
...
# visualize and save
viral_for_map <- rbind(
  viral_bg[, c("Longitude", "Latitude", "Admin")],
...
viral_for_map$Type <- factor(
  viral_for_map$Type,
  levels = c("Point", "Background point")
)
levels(viral_for_map$Type)[
  levels(viral_for_map$Type) == "Point"
] <- "Occurrence"

occ_pt <- geom_point(
  data = viral_for_map[viral_for_map$Type == "Occurrence", ],
...
ggsave(
  filename = paste0("outputs/Figures/SI_Fig2.png"),
  SI_fig2,
  height = 6,
  width = 12,
  dpi = 300
...
ggsave(
  filename = "outputs/Figures/SI_Fig3a.png",
  viPlot_surv[[1]],
  height = 6,
  width = 9,
  dpi = 300
...
ggsave(
  filename = "outputs/Figures/SI_Fig3b.png",
  viPlot_surv[[2]],
  height = 6,
  width = 10,
  dpi = 300
...
plot_gam_spline <- function(df, cov_name) {
  ...
  p1 <- sm |>
    ...
    geom_ribbon(
      aes(ymin = .lower_ci, ymax = .upper_ci, x = !!sym(cov_name)),
      alpha = 0.2
    ) +
...
ggsave(
  filename = paste0("outputs/Figures/SI_Fig4.png"),
  SI_fig4,
  height = 9,
  width = 16,
  dpi = 300
...
ggsave(
  filename = "outputs/Figures/SI_Fig4.png",
  SI_fig5,
  height = 6,
  width = 12,
  dpi = 300
...
SI_fig6 <- surv_tab %>%
  ...
  geom_text(
    aes(
      x = metric,
      y = Region,
      label = paste(sprintf("%.2f", round(value, 2)))
    ),
    color = "black",
    size = 4
  ) +
...
ggsave(
  filename = "outputs/Figures/SI_Fig5.png",
  SI_fig6,
  height = 6,
  width = 6,
  dpi = 300,
...
ggsave(
  filename = "outputs/Figures/SI_Fig7.png",
  AUC_Surv$map,
  height = 6,
  width = 12,
  dpi = 300,
...
ggsave(
  filename = "outputs/Figures/SI_Fig8a.png",
  viPlot_arbo[[1]],
  height = 6,
  width = 9,
  dpi = 300
...
ggsave(
  filename = "outputs/Figures/SI_Fig8b.png",
  viPlot_arbo[[2]],
  height = 8,
  width = 10,
  dpi = 300
...
ggsave(
  filename = "outputs/Figures/SI_Fig9.png",
  SI_fig9,
  bg = "white",
  height = 10,
  width = 10,
...
all_tab <- rbind(base %>% mutate(id = "base"), mod2 %>% mutate(id = "DEN_ae"))
all_tab <- rbind(all_tab, mod3 %>% mutate(id = "DEN_ZIK_ae"))
all_tab$Region <- factor(
  all_tab$Region,
  levels = c(
    "Overall",
    "Endemic",
...
all_tab$disease <- factor(
  all_tab$disease,
  levels = c("dengue", "chikungunya", "zika")
)
  ...
  p <- ggplot(data = all_tab[all_tab$Region == "Overall", ]) +
    ...
    geom_text(
      aes(
        x = id,
        y = disease,
        label = paste(sprintf("%.3f", round(!!sym(metric), 3)))
      ),
      color = "black",
      size = 5
    ) +
...
for (dis in diseases) {
  ...
  p <- ggplot(
    data = all_tab[
      all_tab$disease == dis & !all_tab$Region %in% c("Endemic", "Non-endemic"),
    ]
  ) +
    ...
    geom_text(
      aes(
        x = id,
        y = Region,
        label = paste(sprintf("%.3f", round(meanAUC, 3)))
      ),
      color = "black",
      size = 5
    ) +
...
ggsave(
  filename = "outputs/Figures/SI_Fig10.png",
  SI_fig10,
  height = 9,
  width = 12,
  dpi = 300,
...
SI_fig10 <- ggarrange(
  IQR_plot$DEN + theme(legend.key.width = unit(1, "cm")),
  IQR_plot$CHIK,
  IQR_plot$ZIK,
  IQR_plot$YF,
  ncol = 2,
...
ggsave(
  filename = "outputs/Figures/SI_Fig11.png",
  SI_fig11,
  height = 9,
  width = 18,
  dpi = 300,
...
all_tab$disease <- factor(
  all_tab$disease,
  levels = c("dengue", "chikungunya", "zika", "yf")
)
...
for (metric in metrics) {
  ...
  p <- ggplot(
    data = all_tab[!all_tab$Region %in% c("Endemic", "Non-endemic"), ]
  ) +
    geom_tile(
      aes(x = disease, y = Region, fill = !!sym(metric)),
      color = "white"
    ) +
    ...
    geom_text(
      aes(
        x = disease,
        y = Region,
        label = paste(sprintf("%.2f", round(!!sym(metric), 2)))
      ),
      color = "black",
      size = 3
    ) +
...
ggsave(
  filename = "outputs/Figures/SI_Fig12.png",
  SI_fig12,
  height = 6,
  width = 12,
  dpi = 300,
...
ggsave(
  filename = "outputs/Figures/SI_Fig13.png",
  SI_fig13,
  height = 9,
  width = 18,
  dpi = 300,
...
ggsave(
  filename = "outputs/Figures/SI_Fig14a.png",
  viPlot_yf[[1]],
  height = 6,
  width = 9,
  dpi = 300
...
ggsave(
  filename = "outputs/Figures/SI_Fig14b.png",
  viPlot_yf[[2]],
  height = 8,
  width = 10,
  dpi = 300
...
all_tab_long$metrics <- ifelse(
  all_tab_long$metrics == "meanAUC",
  "AUC",
...
SI_fig14b <- ggplot(
  data = all_tab_long[
    all_tab_long$Region %in% c("Overall", "South America", "Africa"),
  ]
) +
  geom_tile(aes(x = id, y = Region, fill = value), color = "white") +

  geom_text(
    aes(x = id, y = Region, label = paste(sprintf("%.3f", round(value, 3)))),
    color = "black",
    size = 4
  ) +
  scale_y_discrete(limits = rev) +
  ...
  facet_wrap(metrics ~ ., nrow = 1, ncol = 3)
SI_fig14b
...
ggsave(
  filename = "outputs/Figures/SI_Fig15.png",
  SI_fig15,
  height = 8,
  width = 8,
  dpi = 300,
...
all_stack <- stack(
  ...
  arbo_cov_list[[
    -which(
      names(arbo_cov_list) %in%
        c("GDP", "GDP_national", "Urban", "Vaccine", "Vaccine_offset", "Surv")
    )
  ]]
)
...
ggsave(
  filename = "outputs/Figures/SI_Fig16.png",
  SI_fig16,
  height = 20,
  width = 16,
  dpi = 300
...
ggsave(
  filename = "outputs/Figures/SI_Fig17.png",
  SI_fig17,
  height = 18,
  width = 15,
  dpi = 300
...
tsuit <- raster(
  "data/covariate_rasters/Dengue_temperature_suitaiblity_masked_.tif"
)
yf_eye <- raster("data/covariate_rasters/YF_EYE_mask.tif")
...
ggsave(
  filename = "outputs/Figures/SI_Fig18.png",
  SI_fig18,
  height = 10,
  width = 12,
  dpi = 300,
...
# SFig19 ====================================================
unmasked_list <- list.files(
  "outputs/Rasters",
  pattern = "*wmean_unmasked.tif",
  full.names = FALSE
)
unmasked_list
...
SI_fig19 <- ggarrange(
  unmasked_plot$DEN + theme(legend.key.width = unit(1, "cm")),
  unmasked_plot$CHIK,
  unmasked_plot$ZIK,
  unmasked_plot$YF,
  ncol = 2,
...
ggsave(
  filename = "outputs/Figures/SI_Fig19.png",
  SI_fig19,
  height = 9,
  width = 18,
  dpi = 300,
...
# dengue map only ==========================================
Ras <- raster(paste0("outputs/Rasters/DEN_riskmap_wmean_masked.tif"))
...
ggsave(
  filename = "outputs/Figures/Map_dengue_.pdf",
  plot = den_map,
  dpi = 300,
  bg = "white",
  device = "pdf",
